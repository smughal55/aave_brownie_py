...
def main():
    ...
    if network.show_active() in FORKED_LOCAL_ENVIRONMENTS:  # mainnet-fork-aave
        get_weth()
    # ABI
    # Address
    ...
    tx = lending_pool.deposit(
        ...
        {
            "from": account
        },  # account.address is our address as we're depositing collateral for ourselves
    )
    tx.wait(1)
    print("Deposited!")
    # ...how much?
...
def get_asset_price(price_feed_address):
    ...
    latest_price = dai_eth_price_feed.latestRoundData()[
        1
    ]  # price is at index 1, that's the only thing we want
    converted_latest_price = Web3.fromWei(latest_price, "ether")
    print(f"The DAI/ETH price is {converted_latest_price}")
    return float(converted_latest_price)
    # 0.000241816054914095 (18 decimals)
...
def get_borrowable_data(lending_pool, account):
    (
        ...
        account.address
    )  # remember, view functions don't spend any gas
